...
public class DiscoveryFragmentAdapter extends BaseAdapter {
    ...
    private DiscoveryFragmentAdapterHelper mHelper;
...
//            int type = 0;
            ...
            if (type != 3) {
                ...
                for (int j = 0; j < imgCount; j++) {
                    ...
                    imgBean.setHeigh(h);
                    imgBean.setWidth(w);
                    imgBean.setPath(String.valueOf(id));
                    list.add(imgBean);
                }
                bean.setImgs(list);
        ...
        mHelper = new DiscoveryFragmentAdapterHelper(context, onClickListener);
        //处理数据中的imgLine和tags
        mHelper.processImgWallAndTags(mData);
    }
